...
normal_data = NULL    #count_matrix_dir
normal_target = NULL   #normal target 
normal_output = NULL
normal_count = NULL    #number of normal samples

tumor_data = NULL    #count_matrix_dir
tumor_target = NULL    #tumor target
tumor_output = NULL    #output_dir

## all these go in the command
#data = "gtex_tcga_ovary_100.csv"
#normal_count = 100
#normal_target = 1000000
#tumor_target = 2000000
#project = "test1"

if ("-data" %in% args && "-normal_count" %in% args && "-normal_target" %in% args && "-tumor_target" %in% args && "-project" %in% args) {    
  data_index = which(args == "-data")    #Getting index of the "--input" option
  normal_count_index = which(args == '-normal_count')
  normal_target_index = which(args == "-normal_target")
  tumor_target_index = which(args == "-tumor_target")
  project_index = which(args == "-project")
  
  data = args[data_index + 1]
  normal_count = as.numeric(args[normal_count_index + 1]) 
  normal_target = as.numeric(args[normal_target_index + 1])
  tumor_target = as.numeric(args[tumor_target_index + 1])
...
normal_data = data.frame(gene_count_data[, 1:normal_count])
tumor_data = data.frame(gene_count_data[,(normal_count+1):ncol(gene_count_data)])
...
main <- function(counts_matrix_dir, output_dir, target){
  ...
  #store the normal and tumor outputs to later merge them
...
gen_seed <- function(df, condition_list){
  ...
  #print(paste("This is the head of ss: \n ", head(ss)))
  #print(paste("This is the dimension of ss: \n" , dim(ss)))
...
normal_output = main(normal_data, output_dir = paste(project, "_normal_", normal_target , ".csv",sep = ""), target=normal_target)
tumor_output = main(tumor_data, output_dir = paste(project, "_tumor_", tumor_target , ".csv" ,sep = ""), target=tumor_target)

total_data = merge(normal_output, tumor_output, by = "row.names")
rownames(total_data) = total_data[,1]
total_data = total_data[,-1]

write.csv(total_data, paste(project, "_data_subsampled.csv" , sep = ""), row.names=TRUE)
print(paste("Writing total subsampled data to", paste(project, "_data_subsampled.csv" , sep = "")))
